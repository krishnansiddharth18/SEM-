...
def _get_grids():
    ...
    xy_max = 50
    xy_min = -xy_max
    #z_min = 1.2*xy_min+z_shift
    #z_max = 1.2*xy_max+z_shift
    z_min = -60
    z_max = -z_min
...
if __name__ == '__main__':
    ...
    from matplotlib import pyplot as plt
    r = np.array([(_x,_y,_z) for _x in x for _y in y for _z in z])
    ...
    for frame in (0,):        
        ...
        cond = sem.get_conductance('',frame, r).reshape((len(x),len(y),len(z)))         
        sem.run_task(key='',
                    frame=frame,
                    bulk_conductance=sem.bulk_conductivity,
                    time_per_frame=0)
        log_memory(f"Test task complete for frame {frame}")
        potential = sem.get_voltage_map()
        print(potential)
        origin  = np.array((x[0],y[0],z[0]))
        delta   = np.array((x[1]-x[0],y[1]-y[0],z[1]-z[0])) 
        potential_grid = Grid(potential, origin=origin*10, delta=delta*10)
        potential_grid.export("V_grid.dx")
        cond_grid = Grid(cond, origin=origin*10, delta=delta*10)
        cond_grid.export("cond_grid.dx")

        log_memory(f"Test frame {frame} complete")
        #plt.imshow( cond )
        #plt.savefig( f'conductivity_side{frame:04d}.png')
